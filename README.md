...
| <a name="input_sweep"></a> [sweep](#input\_sweep) | Queued-job sweep tuning. GitHub assigns jobs to runners itself, so a VM dispatched for one job can be handed another (label-identical jobs queued together); the leftover job waits for the sweep. rate\_minutes is the EventBridge cadence; min\_age\_seconds is how old a queued job must be before the sweep re-dispatches (long enough to let an in-flight webhook dispatch finish registering — resume is ~3s, runner registration ~30-60s). Worst-case pickup latency for a stolen job is min\_age\_seconds + rate\_minutes. | <pre>object({<br/>    rate_minutes    = optional(number, 1)<br/>    min_age_seconds = optional(number, 90)<br/>  })</pre> | `{}` | no |
